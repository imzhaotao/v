...
const STORY_ANALYSIS_PROMPT = `你是一个专业的电影分镜师。分析以下故事，严格只输出JSON，不要任何思考过程或说明文字。

要求：
1. 根据故事内容自动生成简洁有力的标题（不要"未命名"或"无题"）
...
JSON（严格只输出JSON，不要其他文字）：`;

const SHOT_EXPANSION_PROMPT = `根据以下场景信息，严格只输出2-3个分镜的JSON数组。不要输出任何思考过程、推理说明、思考标签或额外文字。

场景：{location} / {timeOfDay}
描述：{summary}
角色：{characters}

每个分镜字段：sequence(序号), purpose(建立|动作|反应|转场|特写), durationSec(10-25秒), shotType(远景|全景|中景|近景|特写), cameraAngle(平视|仰视|俯视), cameraMovement(固定|推|拉|摇|移|跟), visualDescription(画面描述), emotion(平静|紧张|悬疑|欢快|悲伤|戏剧性)
